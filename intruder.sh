...
function helpPanel(){
	echo -e "\n${yellowColour}[*]${endColour}${grayColour} Uso: ./intruder.sh${endColour}"
...
function dependencies(){
	tput civis
	clear; dependencies=(aircrack-ng macchanger xterm hcxdumptool)
...
function startAttack(){
	...
	if [ "$(echo $attack_mode)" == "Handshake" ]; then
		...
		echo -e "\n\n${yellowColour}[*]${endColour}${grayColour} Obteniendo Hash...${endColour}\n"
		sleep 2
